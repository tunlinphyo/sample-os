...
import { anotherJournalSentences, journalSentences, journalTitles } from "../../../components/keyboard/consts";
import { Modal } from "../../../components/modal";
import { SelectItem } from "../../../components/select";
import { DeviceController } from "../../../device/device";
import { HistoryStateManager } from "../../../device/history.manager";
import { Journal, JournalData, JournalType } from "../../../stores/journal.store";
import { JournalController } from "../journal.controller";


export class JournalEditorPage extends Modal {
    ...
    private init() {
        ...
        this.addEventListener('click', () => {
            ...
            if (!data) return;
            this.journal.saveJournal(data);
        }, this.btnEnd, false);

        this.journal.addChangeListener((status: string) => {
            if (status === "JOURNAL_SAVED") {
                this.closePage();
            }
        });
        ...
        this.device.addEventListener('closeApp', () => {
            const data = this.getData();
            if (data) {
                this.history.updateState(`/journal/edit`, data);
                // else this.history.updateState(`/journal/new`, data);
            }
        });
    }
    ...
    private getData() {
        const list: JournalData[] = [];
        const elems = this.getAllElement('.section');

        for (const elem of elems) {
            const type = elem.dataset.type as JournalType;
    ...
    private getContent(elem: HTMLElement, type: JournalType) {
    ...
    private handleKeyText(key: string) {
        ...
        if (type === 'title') {
            if (journalTitles[key]) focused.textContent = journalTitles[key];
        } else if (this.isParagraph(type)) {
            const insertData = journalSentences[key] || anotherJournalSentences[key];
            if (insertData) {
    ...
    private handleEnter() {
        ...
        const type = parentEl.dataset.type as JournalType;
    ...
    private renderElement(note: JournalData) {
        // @ts-ignore
    ...
    private renderSection(note: JournalData, prevEl?: HTMLElement) {
    ...
    private createSection(prevEl?: HTMLElement, note?: JournalData) {
    ...
    private removeItem(parent: HTMLElement, item: HTMLElement, type: JournalType) {
        if (this.isParagraph(type)) {
